...
// Check exact model ID in your Bedrock console → Model access (it includes a version date suffix)
...
export async function generateExecutiveSummary(
  ...
  try {
    ...
    return fallbackSummary(profile, businessKPIs);
  } catch (error) {
...
function fallbackSummary(profile: Profile, businessKPIs: BusinessKPIs): string {
  return (
    `DecisionIQ analyzed a ${profile.datasetType ?? 'business'} dataset with ${profile.rowCount ?? 'an unknown number of'} rows. ` +
    `The dataset shows total revenue of $${businessKPIs.totalRevenue?.toLocaleString() ?? 'N/A'} and total profit of $${businessKPIs.totalProfit?.toLocaleString() ?? 'N/A'}. ` +
    `The profit margin is ${businessKPIs.profitMargin ?? 'N/A'}%. ` +
    `Data quality score is ${profile.qualityScore ?? 'N/A'}/100.`
  );
}
